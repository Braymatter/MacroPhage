...
fn create_background(
    ...
    let material_handle = materials.add(StandardMaterial {
        base_color_texture: Some(asset_server.load("background_alt_4k.png")),
        unlit: true,
        ..default()
    });
    // adjust quad shape to affect zoom
    let quad_handle = meshes.add(Mesh::from(shape::Quad::new(Vec2::new(64., 64.))));
    commands.spawn_bundle(PbrBundle {
        mesh: quad_handle,
        material: material_handle,
        transform: Transform::from_xyz(0., -20., 0.0)
            .with_rotation(Quat::from_axis_angle(Vec3::X, -PI / 2.0).mul(Quat::from_axis_angle(Vec3::Z, -PI / 2.0 ))),
        ..default()
    });
}
